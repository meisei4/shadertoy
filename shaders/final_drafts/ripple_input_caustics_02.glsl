...
vec4 sample_background_with_height_map(sampler2D tex, vec2 uv);
...
void mainImage(out vec4 frag_color, in vec2 frag_coord) {
    ...
    vec4 background = vec4(0.0);
        ...
        background = sample_background_with_height_map(
            iChannel1, uv 
            //noise_disp_map_1,  
...
vec4 sample_background_with_height_map(
    sampler2D background_tex,
    vec2 uv
    //vec2 disp_map, 
    //float warp_factor
) {
    ...
    vec2 refracted_offset = refracted.xy;// * warp_factor; //TODO figure out how to merge all the displacement layers with the ripple height map layer
